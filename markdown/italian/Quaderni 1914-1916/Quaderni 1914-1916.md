...
Io so che le proposizioni seguenti, così come sono, sono insensate: si può parlare dei numeri, quando vi sono solo cose? Nel caso dunque in cui il mondo consistesse ad esempio di una cosa sola e nient’altro, si potrebbe dire che v’èUNA cosa? Russell probabilmente direbbe: se vi è una cosa, allora vi è anche la funzione ${\displaystyle (\exists x){\hat {\xi }}=x}$. Ma! ––
...
${\displaystyle \left.{\begin{matrix}\sim p\\\sim \sim \sim p\\\sim p\lor \sim p\\\sim p.\sim p\\{\text{etc. etc.}}\end{matrix}}\right\}{\text{e lo stesso vale per la designazione della generalità etc.}}}$
...
Non è forse insensata la definizione dello zero di Russell? Si può in generale parlare di una classe ${\displaystyle {\hat {x}}(x\neq x)}$? Si può quindi parlare di una classe ${\displaystyle {\hat {x}}(x=x)}$? Quindi x ≠ x o x = x è una funzione di x?? Lo zero non dev’esser definito attraverso l’*ipotesi* (∃*ϕ*):(x)\~*ϕ*x? E l’analogo varrebbe per tutti gli altri numeri. Questo getta una luce su tutta la questione relativa all’esistenza di numeri di cose.
...
Secondo la mia definizione delle classi, ${\displaystyle (x).\sim {\hat {x}}(\phi x)}$ è l’affermazione che ${\displaystyle {\hat {x}}(\phi x)}$ è zero, e la definizione dello zero è quindi ${\displaystyle 0={\hat {\alpha }}[(x).\sim \alpha ]{\text{ Def.}}}$
