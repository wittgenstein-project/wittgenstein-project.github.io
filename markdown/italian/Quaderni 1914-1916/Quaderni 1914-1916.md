...
Io so che le proposizioni seguenti, così come sono, sono insensate: si può parlare dei numeri, quando vi sono solo cose? Nel caso dunque in cui il mondo consistesse ad esempio di una cosa sola e nient’altro, si potrebbe dire che v’èUNA cosa? Russell probabilmente direbbe: se vi è una cosa, allora vi è anche la funzione [math]\\displaystyle{ (\\exists x) \\hat{\\xi} = x }[/math]. Ma! ––
...
[math]\\displaystyle{ \\left . \\begin{matrix} \\sim p \\\\ \\sim \\sim \\sim p \\\\ \\sim p \\lor \\sim p \\\\ \\sim p . \\sim p \\\\ \\text{etc. etc.} \\end{matrix} \\right \\} \\text{e lo stesso vale per la designazione della generalità etc.} }[/math]
...
Non è forse insensata la definizione dello zero di Russell? Si può in generale parlare di una classe [math]\\displaystyle{ \\hat{x} (x \\ne x) }[/math]? Si può quindi parlare di una classe [math]\\displaystyle{ \\hat{x}(x = x) }[/math]? Quindi x ≠ x o x = x è una funzione di x?? Lo zero non dev’esser definito attraverso l’*ipotesi* (∃*ϕ*):(x)\~*ϕ*x? E l’analogo varrebbe per tutti gli altri numeri. Questo getta una luce su tutta la questione relativa all’esistenza di numeri di cose.
...
Secondo la mia definizione delle classi, [math]\\displaystyle{ (x). \\sim \\hat{x}(\\phi x) }[/math] è l’affermazione che [math]\\displaystyle{ \\hat{x}(\\phi x) }[/math] è zero, e la definizione dello zero è quindi [math]\\displaystyle{ 0 = \\hat{\\alpha} [(x). \\sim \\alpha ] \\text{ Def.} }[/math]
...
![Illustrazione 11.6.15 no text.png](images/300px-Illustrazione_11.6.15_no_text.png)\
...
![Illustrazione 12.8.16 no text.png](images/Illustrazione_12.8.16_no_text.png)\
...
Disegnando il quadrato ![Illustrazione 4.11.16.png](images/40px-Illustrazione_4.11.16.png)\  nello specchio si nota che lo si può eseguire solo se si prescinde del tutto dal campo visivo e ci si affida esclusivamente alla sensazione muscolare. Si tratta quindi qui di due atti volitivi del tutto diversi. Uno si riferisce alla parte visibile del mondo, l’altro alla parte della sensazione muscolare.
